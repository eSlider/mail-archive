...
func extractBodyText(contentType, transferEncoding string, body io.Reader) string {
	...
	if err != nil {
		// Malformed Content-Type — try reading as plain text.
...
// Returns the first text/plain content, or stripped text/html as fallback.
...
// If the result is still not valid UTF-8 (e.g. no charset was declared), it falls back to
// Windows-1252 decoding as a last resort.
...
// decodeTransferEncoding wraps a reader with the appropriate decoder.
...
// charsetReader wraps r with a decoder that converts from the given charset to UTF-8.
// Returns r unchanged if charset is empty, utf-8, us-ascii, or unknown.
func charsetReader(charset string, r io.Reader) io.Reader {
	...
	enc, err := htmlindex.Get(cs)
	if err != nil || enc == nil {
		return r // unknown charset — return as-is
	}
	return transform.NewReader(r, enc.NewDecoder())
}

// ensureUTF8 converts a string to valid UTF-8. If the string is already valid
// UTF-8 it is returned unchanged. Otherwise it is decoded as Windows-1252
// (the most common legacy encoding for Western European emails).
...
// extractFullBody populates TextBody, HTMLBody and Attachments on a FullEmail.
...
// extractFullMultipart walks multipart parts, collecting text, HTML and attachments.
...
// in the email's subject or body. Returns empty string if no match.
func Snippet(e Email, query string, contextLen int) string {
	...
	// Check subject first (work on runes to get correct offsets).
...
// runeIndex returns the rune-based index of needle in haystack, or -1.
...
// buildSnippet extracts a window around a match position using rune slices.
