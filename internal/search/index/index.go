...
// Search returns emails whose subject or body contains the query.
...
func SearchMulti(accounts []AccountIndex, query string, offset, limit int) SearchResult {
	...
	// (e.g. re-imported PST) appears once. Rows without checksum in path are kept as-is.
			...
			SELECT *,
				ROW_NUMBER() OVER (
					PARTITION BY COALESCE(
						regexp_extract(path, '([0-9a-f]{16})-', 1),
						account_id || '::' || path
					)
